...
namespace MobileStore.Services.Interfaces
{
    internal interface IProductsService
    {
        ...
        Task<List<ProductModel>> GetProductsByIdsAsync(IReadOnlyCollection<int> ids, CancellationToken ct);

        Task ReduceProductStockAsync(IReadOnlyCollection<ProductStockReductionModel> reductions, CancellationToken ct);
    }
}
